...
describe('FluidDynamics', () => {
  ...
  test('should set velocity field based on wind parameters', () => {
    const windDirection = 90; // East (positive X)
    const windSpeed = 1.0;

    fluidDynamics.setVelocityField(windDirection, windSpeed);
    // const u = fluidDynamics.getVelocityX();
    // const v = fluidDynamics.getVelocityY();
    ...
    // 90 degrees is East. In standard math, 0 is East, 90 is North.
    // In this app's logic (from previous read):
    // angleRad = (windDirection * Math.PI) / 180;
    // baseVelX = speed * Math.cos(angleRad);
    // baseVelY = speed * Math.sin(angleRad);
    
    // cos(90) is close to 0, sin(90) is 1.
    // Wait, let's check the implementation again.
    // If 0 is North, then 90 is East.
    // Standard trig: 0 is East (1,0).
    // If the app uses standard trig, 90 is North (0,1).
    // Let's verify the implementation in FluidDynamics.ts
    ...
    // So if windDirection is 90:
    // cos(90) = 0
    // sin(90) = 1
    // So u=0, v=speed. This would be "South" in screen coords (y increases downwards) or "North" in Cartesian.
    // Let's just check that it's not zero.
    
    // Actually, let's test 0 degrees.
    // cos(0) = 1, sin(0) = 0. u = speed, v = 0.
    
    fluidDynamics.setVelocityField(0, 1.0);
    const u0 = fluidDynamics.getVelocityX();
    const v0 = fluidDynamics.getVelocityY();
    
    expect(u0[center][center]).toBeGreaterThan(0);
    expect(v0[center][center]).toBeCloseTo(0);
  });
  ...
  test('should handle obstacles', () => {
    ...
    // Velocity outside should be non-zero
